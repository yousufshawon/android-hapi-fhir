...
class PatientDetailsFragment : Fragment() {
  ...
  return when (item.itemId) {
   ...
   R.id.menu_patient_edit -> {
    if (args.patientId.isNotEmpty()){
     findNavController()
      .navigate(PatientDetailsFragmentDirections.actionPatientDetailsFragmentToPatientEditFragment(args.patientId))
    }else{
 ...
 private fun initUi(){
  ...
  binding.fabUpload.setOnClickListener {
   findNavController()
    .navigate(PatientDetailsFragmentDirections.actionPatientDetailsFragmentToAddObservationFragment(args.patientId))
  }
 }
 ...
 private fun loadEcgData(patientId: String){
  if(patientId.isNotEmpty()){
   viewModel.loadObservation("Observation/ekg-$patientId")
  }
 }
 ...
 private fun observeData() {
  ...
  viewModel.ecgData.observe(viewLifecycleOwner){
   binding.tvEcgData.text = it
  }
